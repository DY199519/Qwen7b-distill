...
Process in the order of "question -> model":
  ...
  3. Write the progress files of each model to disk every SAVE_INTERVAL questions
  4. Use the prompt field from the second file as input
  5. Get direct replies from the original JSON file without repeated API calls

Modification: Adapt to the output format of the second file and extract answers from raw data
"""
...
OUTPUT_FILE_SUFFIX = "2+1-1-test"  # <-- Output file suffix, e.g.: gemini-2.5-flash_answers_2+1-1.json
# =====================================================
...
# JSON file of original questions and answers
...
def load_original_answers(path: Path):
    ...
    if not path.exists():
        print(f"⚠️ Original answer file does not exist: {path}")
        return {}
    ...
    try:
        ...
        print(f"✓ Successfully loaded original answers, containing {len(answers_cache)} questions")
        return answers_cache
        
    except Exception as e:
        print(f"❌ Failed to read original answer file: {e}")
        return {}

def get_direct_answer(question: str, model_name: str, original_answers: dict):
    """Get the direct answer of the specified model to the question from the original answers"""
    if question not in original_answers:
        print(f"  ⚠️ Question not in original answers: {question[:50]}...")
        return ""
    ...
    for model, answer in question_answers.items():
        if model_name.lower() in model.lower() or model.lower() in model_name.lower():
            print(f"  📋 Fuzzy match: {model_name} -> {model}")
            return answer
    
    print(f"  ⚠️ No answer found for model {model_name}")
    return ""
...
def ask(api: OpenAI, model: str, prompt: str, retry: int = 3, pause: int = 2):
    ...
    for i in range(retry):
        try:
            ...
            print(f"⚠️ {model} returned too short/no ending punctuation for the {i+1}th time (len={len(txt)}), retrying...")
        except Exception as e:
            print(f"❌ {model} failed for the {i+1}th time: {e}")
...
if __name__ == "__main__":
    ...
    print(f"\n📚 Number of questions: {len(all_questions)}")
    
    # 2) Read original answer data
    original_answers = load_original_answers(ORIGINAL_JSON)

    # 3) Prepare API, progress files, and row cache for each model
    model_env = {}
    ...
    for qi, q in enumerate(all_questions, 1):
        ...
        for cfg in MODEL_CFGS:
            ...
            # Get the model's direct answer to the question from original answers
            ...
            # Process all combinations of this question
            ...
            # Get from any combination (since all combinations of the same question should have the same third information)
            if q_combinations:
    ...
    print("\n🎉 All processing completed in question order, files saved in:", OUTPUT_DIR)
