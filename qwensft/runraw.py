...
# -*- coding: utf-8 -*-
r"""
部署脚本：读取本地D:\project7\200.txt并上传到远程AutoDL服务器运行原始模型推理
"""
...
REMOTE_QUESTIONS = "/root/200.txt"        # 远程问题文件路径（与第一个脚本相同）
LOCAL_SCRIPT = __file__                   # 本地脚本（仅用于参考）
REMOTE_SCRIPT = "/root/run_raw_inference.py"  # 远程推理脚本路径
REMOTE_LOG = "/root/vicuna_log_raw.out"   # 远程日志路径（不同路径）
MAX_RETRY = 5                             # SSH重试次数
RETRY_GAP = 10                            # 重试间隔秒数
