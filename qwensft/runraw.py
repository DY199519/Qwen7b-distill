...
# -*- coding: utf-8 -*-
r"""
Deployment script: Read local D:\project7\200.txt and upload to remote AutoDL server for original model inference
"""
...
REMOTE_QUESTIONS = "/root/200.txt"        # Remote question file path (same as the first script)
LOCAL_SCRIPT = __file__                   # Local script (for reference only)
REMOTE_SCRIPT = "/root/run_raw_inference.py"  # Remote inference script path
REMOTE_LOG = "/root/vicuna_log_raw.out"   # Remote log path (different path)
...
def main():
    ...
    sftp = client.open_sftp()
    sftp.put("run_raw_inference.py", REMOTE_SCRIPT)
    sftp.close()
    print("✅ Inference script upload completed")
    
    # 5) Check Swift CLI
    _, stdout, _ = client.exec_command('/root/miniconda3/bin/swift --help', timeout=5)
    if stdout.channel.recv_exit_status() == 0:
        print("✓ Swift command-line tool is available")
    else:
        print("⚠ Warning: Swift command not directly detectable")
    ...
    print("🚀 Issuing background execution command ...")
    client.exec_command(cmd)
    print(f"✅ Inference script has started in the background, all outputs are written to {REMOTE_LOG}")
