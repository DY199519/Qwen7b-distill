...
# -*- coding: utf-8 -*-
r"""
Deployment script: Read local file D:\project7\200.txt and upload to remote server for inference
"""
...
REMOTE_SCRIPT = "/root/run_inference_grpo_remote3+1.py"  # Remote inference script path
REMOTE_LOG = "/root/vicuna_log-grpo3+1.out"       # Remote log path
...
def main():
    ...
    sftp = client.open_sftp()
    sftp.put("run_inference_grpo_remote3+1.py", REMOTE_SCRIPT)
    sftp.close()
    ...
    print(f"✅ Inference script started in background, all output written to {REMOTE_LOG}")
    ...
    client.close()
    print(f"\nInference task started, please check {REMOTE_LOG} on the remote server for results")
