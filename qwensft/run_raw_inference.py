...
# -*- coding: utf-8 -*-
r"""
Swift模型批量推理脚本（支持断点续传）
用于在AutoDL服务器上运行原始Qwen/Qwen2.5-7B-Instruct模型推理，处理跨学科问题并保存结果
"""
...
def run_single_inference(question, model_path, timeout=120):
    """
    运行单个问题的推理
    """
...
def run_batch_inference_with_checkpoint(questions, model_path, output_file, input_file):
    """
    批量推理，支持断点续传
    """
    ...
    for idx, (question_index, question) in enumerate(to_process, 1):
        ...
        while retry_count < max_retries:
            answer, command_output = run_single_inference(question, model_path)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        input_file = "/root/200.txt"  # 与第一个脚本相同
    
    output_file = "/root/inference_results_raw.txt"  # 不同路径
