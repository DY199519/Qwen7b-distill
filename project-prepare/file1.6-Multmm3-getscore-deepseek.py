...
------------------------
从 `deepseek_combined_answers.json` 读取每个问题的 top2 / top3 / top4 回答，
进行所有两两配对评分（共 3 组），调用 gpt-4o 进行 5×100 打分并判定胜负。
"""
...
def ask_and_parse(prompt: str,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(f"⚠️ 尝试 {attempt}/{max_attempts} 失败：{e} —— {wait}s 后重试")
            time.sleep(wait)
